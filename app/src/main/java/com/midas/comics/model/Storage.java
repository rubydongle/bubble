...
public class Storage {
    public static abstract class Book implements BaseColumns {
        ...
        public static final String COLUMN_NAME_LOVE = "love";
    ...
    public class ComicDbHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            final String sql = "CREATE TABLE " + Book.TABLE_NAME + " ("
                    ...
                    + Book.COLUMN_NAME_LOVE + " INTEGER DEFAULT 0"
                    + ")";
            db.execSQL(sql);
        }
    ...
    private Comic comicFromCursor(Cursor c) {
        ...
        boolean love = c.getInt(c.getColumnIndex(Book.COLUMN_NAME_LOVE)) != 0;

        return new Comic(this, id, path, name, type, numPages, currentPage, updatedAt, love);
    }
    ...
    public void loveComic(int comicId, boolean love) {
        ContentValues values = new ContentValues();
        values.put(Book.COLUMN_NAME_LOVE, love?1:0);
        String filter = Book.COLUMN_NAME_ID + "=" + Integer.toString(comicId);
        SQLiteDatabase db = mDbHelper.getWritableDatabase();
        db.update(Book.TABLE_NAME, values, filter, null);
    }
